...
public class CommentWriteAdapter extends RecyclerView.Adapter<CommentWriteAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(final ViewHolder holder, int position) {
        if (getItemViewType(position) == TYPE_ADD) {
            holder.mImg.setImageResource(R.drawable.img_addpicture_selected);
            holder.mImg.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    if (mOnAddPicClickListener !=null&&mImgList!=null){
                        // 添加图片
                        mOnAddPicClickListener.onAddPicClick(mImgList.size());
                    }
                }
            });
    ...
    public interface onAddPicClickListener {
        void onAddPicClick(int currentImgSize);
    }
