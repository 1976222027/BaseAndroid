...
import com.daqsoft.update.callback.UpdateCallback;
import com.daqsoft.update.constant.Constants;
import com.daqsoft.update.http.IHttpManager;
import com.daqsoft.update.service.DownloadService;
import com.daqsoft.update.util.PermissionUtils;
    ...
    public void start(){
        if(mConfig!=null && !TextUtils.isEmpty(mConfig.getUrl())){
            ...
            if(mConfig.isShowNotification() && !PermissionUtils.INSTANCE.isNotificationEnabled(mContext)){
                ToastUtils.showLong("请到设置界面打开通知权限查看下载进度!");
